CREATE OR REPLACE VIEW `{project_id}.{dataset}.product_aggregated_label` AS
  SELECT
    ...
    SUM(IF(is_approved = 1 AND in_stock = 1 AND is_targetted = 1, 1, 0)) AS total_targetted,
    SUM(
      IF(is_approved = 1 AND in_stock = 1 AND is_targetted = 1 AND impressions_30_days > 0, 1, 0)
    ) AS total_products_with_impressions_in_last_30_days,
    SUM(
      IF(is_approved = 1 AND in_stock = 1 AND is_targetted = 1 AND clicks_30_days > 0, 1, 0)
