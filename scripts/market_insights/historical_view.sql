...
CREATE OR REPLACE VIEW `{project_id}.{dataset}.market_insights_historical`
WITH
  CountryTable AS (
    SELECT DISTINCT
  ...
  HistoricalProductStatus AS (
    SELECT
      ...
      MAX(IF(LOWER(servability) = 'demoted', TRUE, FALSE)) AS has_demotion_issues,
    FROM (
      SELECT
...
SELECT
  product_view,
  HistoricalData.*,
FROM `{project_id}.{dataset}.product_view_{merchant_id}` product_view
